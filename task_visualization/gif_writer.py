"""Gif writer to save a sequence of frames as a gif.

Usage:
    my_gif_writer = GifWriter('path/to/a/file.gif')
    for frame in my_video:
        my_gif_writer.add(frame)
    my_gif_writer.close()
"""
...
class GifWriter(object):
    ...
    def __init__(self, gif_file, fps=10):
        """Constructor.
    ...
    def close(self):
        """Write the gif."""
        print(
            "Writing gif with {} images to file {}".format(
                len(self._images), self._gif_file
            )
        )
        imageio.mimsave(self._gif_file, self._images, fps=self._fps, loop=0)
