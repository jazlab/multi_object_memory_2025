...
class SlotPartition(abstract_model.AbstractModel):
    ...
    def __init__(
        ...
        **embedding_mlp_kwargs
        ...
        """
        super(SlotPartition, self).__init__(
            dataset=dataset,
        ...
        # Get attention per unit
        self._attention_units = torch.nn.Parameter(
            torch.zeros(self._n_slots, self._dataset.n_units),
            requires_grad=True,
        )

        # Construct embedding MLP
        self.embedding_mlps = torch.nn.ModuleList(
            [
    ...
    def forward(
        ...
        # [batch_size, n_assignments, n_slots, n_units]
        pred_per_assignment_slot = torch.sum(embedding_per_assignment, axis=-3)

        # Add bias and apply nonlinearity
        bias = self.bias_mlp(time=time)  # [batch_size, n_units]
        pred_per_assignment_slot = self._softplus(
            pred_per_assignment_slot + bias.unsqueeze(-2).unsqueeze(-2)
        )

        # [batch_size, n_assignments]
        attention_assignments = self.get_attention_assignments()[trial]

        return (
            pred_per_assignment_slot,
    ...
    def prediction_per_unit_numpy(self, train) -> np.ndarray:
        ...
        """
        # pred_per_assignment_slot has shape [n_trials, n_timesteps, n_assignments, n_slots, n_units]
        # neural and neural_finite have shape [n_trials, n_timesteps, n_units]
        pred_per_assignment_slot, _, neural, neural_finite = self.forward(
        ...
        # [n_slots, n_units]
        attention_units = self.get_attention_units()
        # [n_units]
        max_attn_units = torch.argmax(attention_units, axis=0)

        # Index pred_per_assignment with max_attn_assignments
        # [n_trials, n_timesteps, n_slots, n_units]
        pred_per_slot = pred_per_assignment_slot[
            torch.arange(self.dataset.n_trials), :, max_attn_assignments
        ]

        # Index pred_per_slot with max_attn_units
        # [n_trials, n_timesteps, n_units]
        prediction = pred_per_slot[
            :, :, max_attn_units, torch.arange(self._dataset.n_units)
        ]
    ...
    def loss(self) -> torch.Tensor:
        ...
        """
        # pred_per_assignment_slot has shape [batch_size, n_assignments, n_slots, n_units]
        # attention_assignments has shape [batch_size, n_assignments]
        ...
        (
            ...
            neural,
            neural_finite,
        ) = self.forward(train=True, batch_size=self._batch_size, noise=True)

        # Get loss matrix of shape
        # [batch_size, n_assignments, n_slots, n_units]
        negative_log_likelihood = pred_per_assignment_slot - neural[
            :, None, None
        ] * torch.log(pred_per_assignment_slot)

        # Weight loss by attention
        attention_units = self.get_attention_units()  # [n_slots, n_units]
        attention = (
            attention_assignments[:, :, None, None]
            * attention_units[None, None]
        )
        loss_matrix = negative_log_likelihood * attention

        # Sum loss over assignments and slots
        loss_matrix = torch.sum(loss_matrix, axis=(1, 2))

        # Weight loss matrix by neural_finite and normalize
        loss = torch.sum(loss_matrix * neural_finite) / torch.sum(
            neural_finite
        )
    ...
    def get_attention_units(self):
        # [n_slots, n_units]
        exp_attn = torch.exp(self._attention_units)
        softmax_attn = exp_attn / torch.sum(exp_attn, axis=0, keepdim=True)
        return softmax_attn
